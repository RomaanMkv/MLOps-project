...
def sample_data_cfg(temp_dirs):
    ...
    sample_data_cfg_dict = {
        "sample_data": {
            ...
            "sample_version": "v1",  # Add sample_version here
            "data_format": ".csv",
            # Different sample sizes to test
            "sample_sizes": [0.1, 0.5, 0.8],
            "sample_folder": temp_output_dir.name + "/data/sample_data"
        }
    }
...
def test_sample_data_handles_different_sample_sizes(sample_data_cfg):
    ...
    for sample_size in sample_data_cfg.sample_data.sample_sizes:
        sample_data_cfg.sample_data.sample_size = sample_size
        sample_data(sample_data_cfg)

        # Check that the sample was saved to the expected location
        expected_sample_path = os.path.join(sample_data_cfg.sample_data.sample_folder, "sample.csv")
...
def test_sample_data_saves_sample(sample_data_cfg):
    ...
    sample_data_cfg.sample_data.sample_size = sample_data_cfg.sample_data.sample_sizes[0]
